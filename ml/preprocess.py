...
def load_diabetes_data(file_path: Path) -> pd.DataFrame:
    ...
    if missing_counts.sum() > 0:
        logger.info(
            f"Missing values found in {missing_counts[missing_counts > 0].shape[0]} columns"
        )
...
def preprocess_target(df: pd.DataFrame) -> pd.DataFrame:
    ...
    readmit_mapping = {"NO": 0, "<30": 1, ">30": 0}
...
def process_diagnosis_codes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for diag_col in DIAGNOSIS_COLS:
        if diag_col in df.columns:
            df[f"{diag_col}_is_diabetes"] = (
                df[f"{diag_col}_category"] == "Diabetes"
            ).astype(int)
    ...
    logger.info(
        f"Processed diagnosis codes: created {len(diagnosis_categories_created)} category columns"
    )
    return df
...
def handle_missing_values(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if cols_to_drop:
        logger.info(
            f"Dropping {len(cols_to_drop)} columns with >50% missing: {cols_to_drop}"
        )
...
def feature_engineering(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "num_medications" in df.columns:
        df["high_medication_count"] = (
            df["num_medications"] > df["num_medications"].median()
        ).astype(int)
        df["very_high_medications"] = (
            df["num_medications"] > df["num_medications"].quantile(0.9)
        ).astype(int)
    ...
    if "time_in_hospital" in df.columns:
        ...
        df["very_long_stay"] = (
            df["time_in_hospital"] > df["time_in_hospital"].quantile(0.75)
        ).astype(int)

    # Healthcare complexity features
    if "number_diagnoses" in df.columns:
        df["many_diagnoses"] = (
            df["number_diagnoses"] > df["number_diagnoses"].median()
        ).astype(int)
    ...
    if "age" in df.columns:
        df["is_elderly"] = (
            df["age"].astype(str).str.contains("70|80|90", na=False).astype(int)
        )
        df["is_middle_aged"] = (
            df["age"].astype(str).str.contains("50|60", na=False).astype(int)
        )
        df["is_young"] = (
            df["age"].astype(str).str.contains("10|20|30", na=False).astype(int)
        )
...
def encode_categorical_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    categorical_cols = [
        col for col in df.columns if df[col].dtype == "object" and col != "readmitted"
    ]
    ...
    for col in categorical_cols:
        ...
        if unique_count > 50:
            logger.warning(
                f"{col} has {unique_count} unique values - consider further preprocessing"
            )
...
def final_data_validation(
    df: pd.DataFrame, expect_numeric: bool = False
) -> pd.DataFrame:
    ...
    # Normalise dtypes; object columns are expected unless the frame was one-hot encoded
    for col in feature_cols:
        if df[col].dtype == "bool":
            df[col] = df[col].astype(int)
        elif expect_numeric and df[col].dtype == "object":
...
def preprocess_diabetes_data(
    file_path: Path, output_path: Path = None, encode: bool = False
) -> tuple[pd.DataFrame, list[str]]:
    """
    Complete preprocessing pipeline for diabetes readmission data.

    Categorical columns are kept raw by default (encode=False): one-hot
    encoding belongs to the serving Pipeline built in ml.features, so the
    persisted model can score request-shaped rows directly. Pass encode=True
    only for standalone analysis of a fully numeric frame.
    ...
    # Optional one-hot encoding for standalone analysis
    if encode:
        df = encode_categorical_features(df)

    # Final validation and cleanup
    df = final_data_validation(df, expect_numeric=encode)
